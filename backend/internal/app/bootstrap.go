...
func BootstrapAPI(cfg *config.Config) (*Runtime, error) {
	...
	userService.SetWeChatProfileClient(wechatClient)
	userService.SetWeChatProfileGuard(redisClient)
	userService.SetWeChatAvatarFetcher(user.NewWeChatAvatarFetcher(nil, cfg.WeChat.Timeout))
	authService.SetWeChatProfileSynchronizer(userService)
	...
	adminAuthService.SetLogger(logger)
	...
	adminUserService.SetLogger(logger)
