...
func (s *Service) JoinMatchmaking(ctx context.Context, userID uint64, input JoinMatchmakingInput) (MatchmakingResponse, error) {
	...
	ticket := MatchmakingTicket{
		...
		Player: FriendRoomPlayer{UserID: profile.ID, Nickname: profile.Nickname, Avatar: profile.Avatar, NicknameModerationStatus: profile.NicknameModerationStatus, AvatarModerationStatus: profile.AvatarModerationStatus, Ready: true},
...
func (s *Service) createBotMatch(ctx context.Context, ticket MatchmakingTicket) (MatchmakingTicket, error) {
	...
	difficulty := botDifficultyForRank(ticket.RankTier)
	if difficultyStore, ok := s.rooms.(FriendBotDifficultyStore); ok {
		if err := difficultyStore.SetFriendBotDifficulty(ctx, room.RoomCode, difficulty); err != nil {
			return MatchmakingTicket{}, err
		}
	}
	if lifecycle, ok := s.rooms.(FriendRoomLifecycleStore); ok {
	...
	botTicket := MatchmakingTicket{
		...
		IsBot: true, BotDifficulty: botDifficultyName(difficulty),
	}
...
func publicMatchmakingResponse(ticket MatchmakingTicket) MatchmakingResponse {
	room := sanitizePublicFriendRoom(ticket.Room)
	if room != nil && room.MatchSource == "bot" {
		room.MatchSource = "matchmaking"
	}
	var opponent *FriendRoomPlayer
	if ticket.Opponent != nil {
		copyOpponent := safePublicFriendRoomPlayer(*ticket.Opponent)
		opponent = &copyOpponent
	}
	waitingSeconds := 0
	...
	return MatchmakingResponse{
		...
		MatchID: ticket.MatchID, Room: room, Opponent: opponent,
