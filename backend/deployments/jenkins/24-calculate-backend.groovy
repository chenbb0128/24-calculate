  ...
  stages {
    ...
    stage('构建管理后台') {
      steps {
        sh '''
          set -Eeuo pipefail
          test -f source/frontend/admin-vue/package.json
          docker run --rm \
            --user "$(id -u):$(id -g)" \
            -e NODE_OPTIONS=--openssl-legacy-provider \
            -v "$PWD/source:/workspace" \
            -w /workspace/frontend/admin-vue \
            node:18-alpine \
            sh -ec 'npm install --no-audit --no-fund && npm run test:admin && npm run build:prod'
          rm -rf source/backend/deployments/admin-dashboard
          cp -R source/frontend/admin-vue/dist source/backend/deployments/admin-dashboard
          test -f source/backend/deployments/admin-dashboard/index.html
        '''
      }
    }
