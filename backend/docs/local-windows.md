...
3. 两边分别点击“准备”，确认房间状态从 `waiting` 变为 `ready`；只准备一方时点击开始应被拒绝。
4. 任意一名已准备玩家点击开始，确认返回同一个 `match_id`、`room_seed`、`question_hash` 和 `start_at`；两边题目数字和顺序一致。
5. 倒计时结束后完成一题，确认对手进度会更新；断开/恢复连接时 15 秒内可以继续对局。
